...
func _draw() -> void:
	...
	for raw_obs in _template.get("obstacles", []):
		...
		var opos := obs["pos"] as Vector2
		var osiz := obs["size"] as Vector2
		draw_rect(Rect2(opos - osiz * 0.5, osiz), WALL_COLOR)
		draw_rect(Rect2(opos - osiz * 0.5, osiz), Color(1, 1, 1, 0.06), false, 1.5)
	...
	match room_type:
		...
		"start":
			for dir in exits:
				...
				draw_line(p, p + _dir_vec(dir) * 18, Color(0.6, 0.8, 1.0, 0.7), 2)

func _draw_wall_segments() -> void:
	for dir in ["north", "south", "east", "west"]:
		var segs := _wall_segs(dir, dir in exits)
		for s in segs:
			var wpos := s[0] as Vector2
			var wsiz := s[1] as Vector2
			draw_rect(Rect2(wpos - wsiz * 0.5, wsiz), WALL_COLOR)
...
func _add_healing_zone() -> void:
	...
	zone.body_entered.connect(func(body: Node2D):
		if body.is_in_group("player") and body.get("health") < body.get("max_health"):
			body.set("health", body.get("max_health"))
			GameEvents.player_hit.emit(body.get("max_health"), body.get("max_health"))
	)
...
func _spawn_enemies() -> void:
	...
	for i in ranged:
		var pos: Vector2 = spawns[(melee + i) % spawns.size()] if spawns.size() > 0 else _rand_pos()
...
func _count_enemy_death(_pos: Vector2, _pts: int) -> void:
	_enemies_alive -= 1
	if _enemies_alive <= 0:
		if GameEvents.enemy_died.is_connected(_count_enemy_death):
